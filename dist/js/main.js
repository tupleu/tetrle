...
var Hint;
(function (Hint) {
    Hint[Hint["Wrong"] = 0] = "Wrong";
    Hint[Hint["Half"] = 1] = "Half";
    Hint[Hint["Correct"] = 2] = "Correct";
})(Hint || (Hint = {}));
class Tetromino {
    constructor(type) {
        this._rotation = 0; // 0-3
        this._x = 3;
        this._y = 0;
        this._previous_pos = null;
        this._previous_spin = false; // If this Tetromino has spun since it was last uncovered
        this.type = type;
    }
    get rotation() {
    ...
    convertFinesse() {
        switch (this.type) {
            case 7:
            case 5: {
                if (this._rotation === 3 && this._x === 7)
                    break;
                if (this._rotation === 1 && this._x === 0)
                    break;
...
class Board {
    constructor() {
        ...
        for (var i = 0; i < Board.BOARD_HEIGHT; ++i) {
            this._board.push(new Array(Board.BOARD_WIDTH).fill(0));
        }
    }
    ...
    validPosition(tetromino, rotation = Rotation.None, x_translation = 0, y_translation = 0) {
    ...
    checkLineClears() {
        for (var y = 0; y < this._board.length; ++y) {
            ...
            this._board.unshift(new Array(Board.BOARD_WIDTH).fill(0));
        }
    }
    translatePiece(tetromino, x_translation, y_translation) {
        if (!this.validPosition(tetromino, Rotation.None, x_translation, y_translation)) {
            return;
        }
    ...
    rotatePiece(tetromino, rotation) {
        if (this.validPosition(tetromino, rotation)) {
            tetromino.rotate(rotation);
            this.checkSlide(tetromino);
            return;
        }
        const tests = tetromino.rotationTests(rotation);
        for (const test of tests) {
            const x_translation = test[0];
            const y_translation = test[1] * -1;
    ...
    addPiece(tetromino) {
        ...
        if (!this.validPosition(tetromino))
            return new_board;
    ...
    dasPiece(tetromino, x_shift = 0, y_shift = 1) {
        var x = 0;
        var y = 0;
...
class Game {
    constructor() {
        ...
        this._current_piece_index = null;
        this._guess_num = 0;
        this._hints = [];
        this._solution_pieces = [];
        this._mouse_x = 0;
        this._mouse_y = 0;
    ...
    translatePiece(x_translation, y_translation) {
        if (!this._current_piece)
            return;
    ...
    addPiece() {
        ...
        this._boards[this._guess_num].push(this._current_board);
        if (this._current_piece_index === null)
            return;
    ...
    undoPiece() {
        var _a;
        (_a = this._current_piece) === null || _a === void 0 ? void 0 : _a.reset();
        ...
        if (this._boards[this._guess_num].length === 1)
            return; // shouldn't have collisions on empty board
    ...
    guessRow() {
        if (this._current_piece || this._current_piece_index !== this._pieces[this._guess_num].length)
            return false;
        ++this._guess_num;
        this._current_board = new Board();
        this._boards.push([]);
    ...
    drawSelect() {
        ...
        if ((element === null || element === void 0 ? void 0 : element.className) !== 'tile-piece' && (element === null || element === void 0 ? void 0 : element.className) !== 'tile')
...
Game.rotation_names = ['A', 'B', 'C', 'D'];
var I;
(function (I) {
    I[I["clockwise"] = 0] = "clockwise";
    I[I["counterclockwise"] = 1] = "counterclockwise";
    I[I["left"] = 2] = "left";
    I[I["right"] = 3] = "right";
    I[I["das_left"] = 4] = "das_left";
    I[I["das_right"] = 5] = "das_right";
    I[I["drop"] = 6] = "drop";
    I[I["down"] = 7] = "down";
})(I || (I = {}));
class SolutionPiece {
    constructor(type, instructions = null, prerequisites = null) {
        this.type = type;
...
class Solution {
    ...
    static add_piece(solution, solution_pieces, solution_piece) {
    ...
    static shuffle_bag(bag) {
        return bag.map(value => ({ value, sort: Math.random() }))
            .sort((a, b) => a.sort - b.sort)
            .map(({ value }) => value);
    }
    static flip_type(type) {
        if (type === Type.I || type === Type.O || type === Type.T)
            return type;
        if (type === Type.S)
            return Type.Z;
        if (type === Type.Z)
    ...
    static flip_instruction(instruction) {
        if (instruction === I.left)
            return I.right;
        if (instruction === I.right)
            return I.left;
        if (instruction === I.das_left)
            return I.das_right;
        if (instruction === I.das_right)
            return I.das_left;
        if (instruction === I.clockwise)
            return I.counterclockwise;
        if (instruction === I.counterclockwise)
            return I.clockwise;
        return instruction;
    }
    static flip_instructions(piece) {
        if (piece.type === Type.O || piece.type === Type.I) {
            return piece.instructions ? piece.instructions.map(Solution.flip_instruction) : null;
        }
        if (!piece.instructions)
            return null;
        const new_instructions = piece.instructions.map(Solution.flip_instruction);
    ...
    static flip_piece(piece) {
        var _a;
        return new SolutionPiece(Solution.flip_type(piece.type), Solution.flip_instructions(piece), (_a = piece.prerequisites) === null || _a === void 0 ? void 0 : _a.map(Solution.flip_type));
    }
    static flip_bag(bag) {
        return bag.map(Solution.flip_piece);
    }
    static get_new_solution(solution, solution_pieces, bag, flip, order = false) {
        const new_bag = [];
        // while(bag.length > 0) {
        const n = bag.length;
        if (n === 0)
    ...
    static TKI_Fonzie(bag) {
        const variation = Math.floor(Math.random() * 2);
        switch (variation) {
            default:
            case 0: { // 6-height PC
                console.log('6-Height PC');
                const variation2 = Math.floor(Math.random() * 6);
                switch (variation2) {
                    case 0: {
                        const PC_end = Math.floor(Math.random() * 2);
            ...
            case 1: { // LST
                console.log('LST');
                const variation2 = Math.floor(Math.random() * 2);
                switch (variation2) {
                    case 0: {
    ...
    static TKI_Fonzie_PC6(bag, variation) {
        switch (variation) {
            case 'FONZIE_PCO': {
    ...
    static TKI_FlatTop(bag) {
        const variation = Math.floor(Math.random() * 3);
        switch (variation) {
            case 0: { // LST
                console.log('LST');
                const variation2 = Math.floor(Math.random() * 9);
                switch (variation2) {
                    case 0: {
            ...
            case 2: { // 8-height PC
                ...
                const variation2 = Math.floor(Math.random() * 5);
                switch (variation2) {
                    case 0: {
                        bag.push(new SolutionPiece(Type.Z, 'A4'));
                        bag.push(new SolutionPiece(Type.S, 'D1'));
    ...
    static TKI_FlatTop_PC8(bag, variation) {
        switch (variation) {
            case 'FLAT_8PC_S': {
                const variation2 = Math.floor(Math.random() * 3);
                switch (variation2) {
                    case 0: {
            ...
            case Type.O: {
                ...
                switch (variation) {
                    case 0: {
                        ...
                        const variation3 = Math.floor(Math.random() * 7);
                        switch (variation3) {
                            case 0: {
                                bag.push(new SolutionPiece(Type.L, 'A8'));
                                return 'PC6_O_LA8';
                    ...
                    case 1: {
                        ...
                        bag.push(new SolutionPiece(Type.S, 'B2', [Type.J]));
                        const variation2 = Math.floor(Math.random() * 11);
                        switch (variation2) {
                            case 0: {
            ...
            case Type.I: {
                ...
                switch (variation) {
                    case 0: {
                        ...
                        bag.push(new SolutionPiece(Type.Z, 'A3', [Type.J]));
                        bag.push(new SolutionPiece(Type.S, 'D1', [Type.J]));
                        const variation2 = Math.floor(Math.random() * 12);
                        switch (variation2) {
                            case 0: {
                                bag.push(new SolutionPiece(Type.T, 'D5', [Type.Z]));
    ...
    static answer1(solution_pieces) {
        console.log('TKI');
    ...
    //     return solution
    // }
    static answer4(solution_pieces) {
    ...
    static PC1_triangle(bag, position) {
        switch (position % 4) {
            case 0: {
    ...
    static PC2_fill(bag, sum) {
        //I=1, J=2, L=3, O=4, S=5, T=6, Z=7
        switch (sum) {
            case 124: { // T1
                const variation = Math.floor(Math.random() * 7);
                var variation2 = Math.floor(Math.random() * 2);
                switch (variation) {
                    case 0: {
                        bag.push(new SolutionPiece(Type.O, 'A2'));
    ...
    static answer5(solution_pieces) {
        console.log('PCO');
    ...
    static answer6(solution_pieces) {
        const flip = Math.random() < 0.5;
        var solution = new Board();
        var bag = [];
    ...
    static answer7(solution_pieces) {
        const flip = Math.random() < 0.5;
        var solution = new Board();
        var bag = [];
        bag.push(new SolutionPiece(Type.O, [I.right]));
    ...
    static answer0(solution_pieces) {
        const flip = Math.random() < 0.5;
        var solution = new Board();
        var bag = [];
...
function keypressed(key) {
    switch (key) {
        ...
        case 'arrowleft':
        case "a": {
            if (pressed_keys.has('d') || pressed_keys.has('arrowright'))
                break;
        ...
        case 'arrowright':
        case "d": {
            if (pressed_keys.has('a') || pressed_keys.has('arrowleft'))
                break;
            game.translatePiece(1, 0);
            break;
        }
...
const pressed_keys = new Map(); //implement input of positions
